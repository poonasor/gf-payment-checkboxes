...
(function($) {
    ...
    function initCheckboxProducts() {
        ...
        $(document).on('change', '.gfield-checkbox-product-choice', function() {
            ...
            if ($form.length) {
                ...
                if (formId) {
                    console.log('[GF Checkbox Products] Checkbox changed, recalculating form', formId);
    ...
    function initializeForm(formId) {
        ...
        if (typeof window.gform !== 'undefined' && window.gform.addFilter) {
            ...
            window.gform.addFilter('gform_product_total', function(total, formIdParam) {
                if (parseInt(formIdParam) === parseInt(formId)) {
                    var checkboxTotal = calculateCheckboxProductsTotal(formId);
                    console.log('[GF Checkbox Products] Adding checkbox total:', checkboxTotal, 'to existing total:', total);
            ...
            window.gform.addFilter('gform_product_subtotal', function(subtotal, formIdParam) {
                if (parseInt(formIdParam) === parseInt(formId)) {
                    var checkboxTotal = calculateCheckboxProductsTotal(formId);
    ...
    function calculateCheckboxProductsTotal(formId) {
        var total = 0;
        var selector = '#gform_' + formId + ' .gfield-checkbox-product-choice:checked';
    ...
    function recalculateTotal(formId) {
        ...
        $('#gform_' + formId + ' .ginput_quantity, #gform_' + formId + ' .gfield_price').first().trigger('change');
    }
    ...
    function updateTotalField(formId) {
        ...
        var otherTotal = 0;
        $form.find('.gfield_price').not('.gfield-checkbox-product-choice').each(function() {
